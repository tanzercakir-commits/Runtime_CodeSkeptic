...
| RS-VM-0005 | Requested size is not a multiple of the host allocation granularity | `info` / `high` | capability present but required property absent | `PROVEN` | `SUPPORTED` / `UNSUPPORTED` | The host rounds the reservation up. Two forms since T-019 (2026-07-30): with no declared reliance, an `info` note on a `SUPPORTED` verdict; with `relies_on_unmapped_beyond_size` declared, `high` on `UNSUPPORTED` — the bytes past the requested size are addressable in every execution, so the declared guarantee holds in none. |
...
Severity answers *how bad is this if it is real?* Confidence answers *how sure are we?* Neither constrains the other. `RS-VM-0008` is `high` severity and `HYPOTHESIS` confidence — a serious fragility that is not a present defect. `RS-VM-0005` is `PROVEN` in both of its forms and its severity is the caller's own claim: `info` when no reliance on the unmapped tail is declared (certainly true, recorded, harmless), `high` when `relies_on_unmapped_beyond_size` is declared (certainly true, and the declared guarantee holds in no execution). The false-positive campaign forced this split: the single `medium`/`CONDITIONALLY_SUPPORTED` form fired on 42% of all real mappings, which is correct and, in a gate, indistinguishable from noise.
