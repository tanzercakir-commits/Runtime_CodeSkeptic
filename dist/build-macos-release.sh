...
cp contracts/*.json "$D/contracts/" 2>/dev/null || true
...
# Prove it runs before packaging it.
"$D/bin/rs-check" contracts/campaign/redis-jemalloc-page-size-lg12.json \
    --profile profiles/measured/macos-14-arm64-native.measured.json >/dev/null && \
    echo "self-check: rs-check ran (verdict UNSUPPORTED, exit 1 expected below)" || true
